...
But, of course, the question is, how do we simulated multiple, interacting populations?
...
And we can slurp that up if we want:
```{r}
slurp3 <- slurp_spip(three_pop_dir, num_generations = 2)
```
And investigate census sizes, etc.
```{r}
...
0, 1, and 2, according the order in which they were passed to spip (i.e., their order
...
Here, we will imagine different rates of self-recruitment of the larve, which means different rates
at which surviving larve are going to be migrants.
The populations are labelled 0, 1, and 2, and, as stated above, we will simulate
...
```
And slurp up the output:
```{r}
slurp_m <- slurp_spip(mig_dir, num_generations = 2)
```
